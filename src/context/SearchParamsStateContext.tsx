...
interface SearchParamsStateProviderProps {
  children: ReactNode;
  pathname?: string;
  resetParamsOnChange?: string[];
}
...
const SearchParamsStateContext = createContext<SearchParamsStateContextValue | undefined>(undefined);
...
export default function SearchParamsStateProvider({
  children,
  pathname,
  resetParamsOnChange = defaultResetParamsOnChange,
}: SearchParamsStateProviderProps) {
...
function useSearchParamsStateContext() {
  const context = useContext(SearchParamsStateContext);
