...
import { SVGCart } from "../Svg/Svg.component";
...
export default function GlobalHeader() {
  ...
  useEffect(() => {
    ...
    function scrollEvent() {
      ...
      if (st > lastScrollTop) {
        if (!globalNavIsHidden && st > 80) {
          globalNavIsHidden = true;
          setIsStickyHidden(true);
        }
      }
      else if (st < lastScrollTop) {
        if (globalNavIsHidden) {
          globalNavIsHidden = false;
  ...
  return (
    <div className="relative block h-20 w-full bg-white">
      <div className={globalHeaderVariants({ hidden: isStickyHidden })}>
